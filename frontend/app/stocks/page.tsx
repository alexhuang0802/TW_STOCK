...
interface Analyst {
  ...
  recMean: number; recKey: string; recText: string; recColor: string;
  numAnalysts: number; currentPrice: number | null; currency: string;
  upside: number | null;
...
function AnalystPanel({ data, loading }: { data: Analyst | null; loading: boolean }) {
  if (loading) {
    return (
      <div className="bg-gray-900/70 border border-gray-700/50 rounded-xl p-4 space-y-3 animate-pulse">
        <div className="h-3 bg-gray-800 rounded w-1/3" />
        <div className="h-8 bg-gray-800 rounded w-2/3" />
        <div className="h-2 bg-gray-800 rounded w-full" />
        <div className="grid grid-cols-3 gap-2">
          {[0,1,2].map(i => <div key={i} className="h-10 bg-gray-800 rounded" />)}
        </div>
      </div>
    );
  }
  if (!data || !data.numAnalysts) {
    return (
      <div className="bg-gray-900/70 border border-gray-700/50 rounded-xl p-4">
        <div className="text-[10px] text-gray-500 uppercase tracking-wider mb-2">分析師評等</div>
        <p className="text-xs text-gray-600">此股票暫無分析師評等資料</p>
      </div>
    );
  }

  const fmt = (n: number | null) => n == null ? '-' : n >= 1000 ? n.toLocaleString('en-US', { maximumFractionDigits: 0 }) : n.toFixed(2);
  const total = data.counts.strongBuy + data.counts.buy + data.counts.hold + data.counts.sell + data.counts.strongSell || 1;

  const bars = [
    { label: '強力買入', count: data.counts.strongBuy,  color: 'bg-emerald-500' },
    { label: '買入',     count: data.counts.buy,        color: 'bg-green-500'   },
    { label: '中立',     count: data.counts.hold,       color: 'bg-yellow-500'  },
    { label: '賣出',     count: data.counts.sell,       color: 'bg-orange-500'  },
    { label: '強力賣出', count: data.counts.strongSell, color: 'bg-red-500'     },
  ];

  // recMean: 1=strong buy … 5=strong sell. Map to 0-100% position
  const needlePos = ((data.recMean - 1) / 4) * 100;

  return (
    <div className="bg-gray-900/70 border border-gray-700/50 rounded-xl p-4 space-y-3">
      <div className="text-[10px] text-gray-500 uppercase tracking-wider">分析師評等 · {data.numAnalysts} 位分析師</div>

      {/* Consensus */}
      <div className="flex items-center justify-between">
        <span className={`text-xl font-bold ${data.recColor}`}>{data.recText}</span>
        <div className="text-right">
          <div className="text-[10px] text-gray-500">共識評分</div>
          <div className="text-sm font-semibold text-gray-300">{data.recMean.toFixed(2)} / 5.00</div>
        </div>
      </div>

      {/* Needle bar: strong buy ← → strong sell */}
      <div className="space-y-1">
        ...
        <div className="relative h-2">
          <div className="absolute top-0 w-0.5 h-3 bg-white rounded-full shadow -translate-y-0.5"
            style={{ left: `clamp(0%, ${needlePos}%, 100%)` }} />
        </div>
        <div className="flex justify-between text-[9px] text-gray-600">
          <span>強力買入</span><span>中立</span><span>強力賣出</span>
        </div>
      </div>
      ...
      <div className="space-y-1.5">
        {bars.map(b => (
          <div key={b.label} className="flex items-center gap-2">
            <span className="text-[10px] text-gray-500 w-14 flex-shrink-0">{b.label}</span>
            <div className="flex-1 h-1.5 bg-gray-800 rounded-full overflow-hidden">
              <div className={`h-full rounded-full ${b.color}`} style={{ width: `${b.count / total * 100}%` }} />
            </div>
            <span className="text-[10px] text-gray-400 w-4 text-right flex-shrink-0">{b.count}</span>
          </div>
        ))}
      </div>
      ...
      <div className="border-t border-gray-700/50 pt-3">
        <div className="text-[10px] text-gray-500 mb-2">目標價 ({data.currency})</div>
        <div className="grid grid-cols-3 gap-2">
          {[
            { label: '最低', val: data.targetLow,  color: 'text-red-400'   },
            { label: '平均', val: data.targetMean, color: 'text-blue-300'  },
            { label: '最高', val: data.targetHigh, color: 'text-green-400' },
          ].map(({ label, val, color }) => (
            <div key={label} className="bg-gray-800/50 rounded-lg p-2 text-center">
              <div className="text-[9px] text-gray-500 mb-0.5">{label}</div>
              <div className={`text-sm font-bold ${color}`}>{fmt(val)}</div>
            </div>
          ))}
        </div>
        {data.upside != null && (
          <div className="mt-2 text-center">
            <span className="text-[10px] text-gray-500">現價距均值目標 </span>
            <span className={`text-xs font-bold ${data.upside >= 0 ? 'text-green-400' : 'text-red-400'}`}>
...
export default function StocksPage() {
  ...
  const fetchAnalyst = useCallback(async (symbol: string) => {
    ...
    try {
      const res = await fetch(`/api/stocks/analyst?symbol=${symbol}`);
      const data = await res.json();
      if (!data.error) setAnalyst(data);
    } catch { /* ignore */ }
  ...
  return (
    <div className="h-screen bg-[#0d1117] text-white flex flex-col overflow-hidden">
      {/* ── Header ── */}
      <header className="border-b border-gray-800/60 flex-shrink-0 bg-[#0d1117]/95 backdrop-blur z-50">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* ── Left Sidebar ── */}
        <aside className="w-64 flex-shrink-0 border-r border-gray-800 flex flex-col bg-[#0d1117]">
            ...
            <div className="flex gap-2">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full pl-7 pr-3 py-1.5 bg-gray-800 border border-gray-700 rounded-lg text-xs text-white placeholder-gray-600 focus:outline-none focus:ring-1 focus:ring-blue-500"
                />
              </div>
              <button onClick={() => searchInput.trim() && addStock(searchInput.trim())}
                className="w-7 h-7 rounded-lg bg-blue-600 hover:bg-blue-500 flex items-center justify-center text-white font-bold text-sm transition-colors flex-shrink-0">+</button>
            </div>
            <div className="flex gap-1">
              {(['all', 'tw', 'us'] as const).map(m => (
                <button key={m} onClick={() => setMarket(m)}
                  className={`flex-1 py-1 rounded text-xs font-medium transition-colors ${market === m ? 'bg-blue-600 text-white' : 'bg-gray-800 text-gray-500 hover:text-white'}`}>
                  {m === 'all' ? '全部' : m === 'tw' ? '台股' : '美股'}
                </button>
              ))}
            </div>
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              ...
              <div className="text-center py-10 text-gray-600 text-xs px-4">搜尋後按 + 新增股票</div>
            ) : (
              filtered.map(symbol => {
                ...
                return (
                  <div key={symbol}
                    ...
                    className={`flex items-center gap-2 px-3 py-2.5 cursor-pointer border-l-2 transition-colors ${isSelected ? 'bg-blue-950/30 border-l-blue-500' : 'border-l-transparent hover:bg-gray-800/40'}`}>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5">
                        <span className={`font-bold text-sm ${isSelected ? 'text-blue-300' : 'text-white'}`}>{base(symbol)}</span>
                        <span className={`text-[9px] px-1 py-0.5 rounded font-medium ${isTW(symbol) ? 'bg-blue-900/50 text-blue-400' : 'bg-purple-900/50 text-purple-400'}`}>
                          {isTW(symbol) ? 'TW' : 'US'}
                        </span>
                      </div>
                      <div className="text-[10px] text-gray-500 truncate">{q ? displayName(q) : '載入中…'}</div>
                    </div>
                    {q && (
                      <div className="text-right flex-shrink-0">
                        <div className="text-xs font-semibold text-gray-200">{fmtPrice(q.regularMarketPrice)}</div>
                        <div className={`text-[10px] font-medium ${pos ? 'text-green-400' : 'text-red-400'}`}>
                    ...
                    <button onClick={e => { e.stopPropagation(); removeStock(symbol); }}
                      className="flex-shrink-0 w-5 h-5 rounded hover:bg-red-900/40 text-gray-700 hover:text-red-500 transition-colors text-xs flex items-center justify-center opacity-0 group-hover:opacity-100">×</button>
                  </div>
                );
              })
            )}
          ...
          {lastUpdate && (
            <div className="p-3 border-t border-gray-800 text-[10px] text-gray-600">
              更新 {lastUpdate.toLocaleTimeString('zh-TW')} · 60s 自動刷新
            </div>
          )}
        </aside>
        ...
        <main className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          <div className="bg-gray-900/70 border border-gray-700/50 rounded-xl p-4">
            ...
            {selectedQuote ? (
              <>
                ...
                <div className="text-2xl font-bold tracking-tight">{selectedQuote.currency ?? ''} {fmtPrice(selectedQuote.regularMarketPrice)}</div>
                ...
                <div className="text-[10px] text-gray-600 mt-1.5">Yahoo Finance · {lastUpdate?.toLocaleTimeString('zh-TW')}</div>
              </>
            ) : (
              <div className="text-sm text-gray-500 mt-2">← 點左側自選股查看詳情</div>
            )}
          </div>
          ...
          <div className="bg-gray-900/70 border border-gray-700/50 rounded-xl p-4">
            ...
            <div className="text-xs font-semibold text-gray-300 mb-2">自選股脈動</div>
            {breadthQuotes.length > 0 ? (
              <div className="flex items-end gap-3">
                <div className="text-center"><div className="text-[10px] text-gray-500 mb-1">上漲</div><div className="text-2xl font-bold text-green-400">{upCount}</div></div>
                <div className="text-center"><div className="text-[10px] text-gray-500 mb-1">下跌</div><div className="text-2xl font-bold text-red-400">{downCount}</div></div>
                <div className="text-center ml-auto">
                  <div className="text-[10px] text-gray-500 mb-1">平均</div>
                  <div className={`text-xl font-bold ${avgChange >= 0 ? 'text-green-400' : 'text-red-400'}`}>{avgChange >= 0 ? '+' : ''}{avgChange.toFixed(2)}%</div>
                </div>
              </div>
          ...
          {selectedQuote ? (
            <div className="bg-gray-900/70 border border-gray-700/50 rounded-xl p-4 grid grid-cols-2 gap-2">
              {[
                ...
                ['52W 高', selectedQuote.fiftyTwoWeekHigh ? fmtPrice(selectedQuote.fiftyTwoWeekHigh) : '-'],
              ].map(([label, val]) => (
                <div key={label}>
                  <div className="text-[10px] text-gray-600">{label}</div>
                  <div className="text-xs font-semibold text-gray-200">{val}</div>
                </div>
              ))}
            </div>
          ) : (
            <div className="bg-gray-900/70 border border-gray-700/50 rounded-xl p-4">
              ...
              <p className="text-xs text-gray-600">把即時行情、技術指標整理成結構化快照，方便 AI 輔助分析。</p>
            </div>
          )}
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 xl:grid-cols-[1fr_320px] gap-3">
                ...
                <div className="space-y-3">
                  ...
                  <AnalystPanel data={analyst} loading={analystLoading} />
                  {news.length > 0 && (
                    <div className="bg-gray-900/70 border border-gray-700/50 rounded-xl p-4">
              ...
              <div className="bg-gray-900/70 border border-gray-700/50 rounded-xl p-4">
                ...
                {finLoading ? (
                  <div className="grid grid-cols-4 sm:grid-cols-8 gap-2">
                    {Array.from({ length: 16 }).map((_, i) => <div key={i} className="bg-gray-800/50 rounded-lg px-3 py-2.5 animate-pulse"><div className="h-2 bg-gray-700 rounded w-2/3 mb-2" /><div className="h-3 bg-gray-700 rounded" /></div>)}
                  </div>
                ) : financials ? (
                  <div className="grid grid-cols-4 sm:grid-cols-8 gap-2">
                    {([
                      ['市值', financials.marketCap], ['本益比', financials.trailingPE], ['預期本益比', financials.forwardPE], ['EPS', financials.eps],
                      ['營收', financials.revenue], ['營收成長', financials.revenueGrowth], ['毛利率', financials.grossMargin], ['淨利率', financials.profitMargin],
                      ['ROE', financials.roe], ['ROA', financials.roa], ['負債/權益', financials.debtToEquity], ['流動比率', financials.currentRatio],
                      ['股價淨值比', financials.priceToBook], ['殖利率', financials.dividendYield], ['Beta', financials.beta], ['空頭比率', financials.shortRatio],
                ...
                <div className="bg-gray-900/70 border border-gray-700/50 rounded-xl p-4">
                  ...
                  {indicLoading ? (
                    <div className="grid grid-cols-4 sm:grid-cols-7 gap-1.5">
                      {Array.from({ length: 14 }).map((_, i) => <div key={i} className="bg-gray-800/50 rounded-lg px-3 py-2.5 animate-pulse"><div className="h-2 bg-gray-700 rounded w-2/3 mb-2" /><div className="h-3 bg-gray-700 rounded" /></div>)}
                    </div>
                  ) : (
                    <div className="grid grid-cols-4 sm:grid-cols-7 gap-1.5">
